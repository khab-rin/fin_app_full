use crate::primitives::composite::implements::Fio;
use crate::primitives::frozen::text::{FirstName, MidName, Snils, SurName};
use crate::primitives::frozen::text_base::String1_25;
use crate::service::mchd::service::{NewMchdData, MchdType};
use crate::service::mchd::implements::{
    RussDocumCode,
    Delegate,
    DelegateType,
    DelegateWrap,
    PersonDocum,
    PersonMchd,
    WrapPerson
};

pub fn make_delegate_wrap(
    data: &NewMchdData
) -> DelegateWrap {

    DelegateWrap {
        delegate_type: DelegateType::PhysicalPerson,
        delegate: make_delegate(data)
    }
}

pub fn make_delegate(
    data: &NewMchdData
) -> Delegate {

     Delegate {
        organization: None,
        ip: None,
        person: Some(make_delegate_wrap_person(data)),
        filial: None,
        foreign_organization: None
    }
}

pub fn make_delegate_wrap_person(
    data: &NewMchdData
) -> WrapPerson {

    WrapPerson {
        principal_notarial_status: None,
        inn: Some(data.user_inn.clone()),
        snils: Some(Snils::unchecked(data.user_snils.beat_string())),
        position: None,
        direct_authority_doc: None,
        person: make_delegate_person_mchd(data)
    }
}


pub fn make_delegate_person_mchd(
    data: &NewMchdData
) -> PersonMchd {

    let is_citizen = match data.mchd_type {
        MchdType::FnsMchd => Some(data.manager_is_citizen),
        _ => None
    };

    PersonMchd {
        gender: None,
        is_citizen,
        ern_num: None,
        birth_day: Some(data.user_birth_day.clone()),
        birth_place: None,
        country_code: None,
        tel_number: None,
        email: None,
        fio: Fio {
            sur_name: SurName::unchecked(data.user_sur_name.beat_string()),
            first_name: FirstName::unchecked(data.user_first_name.beat_string()),
            mid_name: Some(MidName::unchecked(data.user_mid_name.beat_string())),
        },
        address: None,
        person_docums: Some(make_delegate_passport(data))
    }
}


pub fn make_delegate_passport(
    data: &NewMchdData
) -> PersonDocum {

    PersonDocum {
        doc_code: RussDocumCode::PasspRf,
        doc_ser_num: String1_25::unchecked(data.user_passport_number.beat_string()),
        doc_date: data.user_passport_issue_date.clone(),
        issued_by: Some(data.user_passport_issueer.clone()),
        issued_code: Some(data.user_passport_ussuer_code.clone()),
        exp_date: None

    }


}
