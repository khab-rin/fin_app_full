...
pub enum HomeMchdPower {
    ...
    #[serde(rename = "Просматривать сведения о персонале")]
    H110,
    #[serde(rename = "Редактировать сведения о персонале (полный доступ)")]
    H111,

    #[serde(rename = "Просматривать выписки и банковские счета")]
    H210,
    #[serde(rename = "Распоряжаться банковскими счетами и подписывать платежи (полный доступ)")]
    H211,

    #[serde(rename = "Просматривать первичные документы и налоговую отчетность")]
    H310,
    #[serde(rename = "Подписывать и отправлять отчетность, акты, УПД и счета-фактуры (полный доступ)")]
    H311,

    #[serde(rename = "Просматривать складские остатки и накладные")]
    H410,
    #[serde(rename = "Управлять складом, подписывать товарно-транспортные накладные и акты приемки (полный доступ)")]
    H411,

    #[serde(rename = "Просматривать договоры и юридические документы")]
    H510,
    #[serde(rename = "Заключать, изменять, расторгать договоры и подписывать соглашения (полный доступ)")]
    H511,

    #[serde(rename = "Просматривать журнал событий безопасности и результаты проверок контрагентов")]
    H610,
    #[serde(rename = "Управлять уровнями риска, блокировать учетные записи и согласовывать доступы (полный доступ)")]
    H611,

    #[serde(rename = "Просматривать системные настройки и конфигурации")]
    H710,
    #[serde(rename = "Администрировать информационные системы, управлять интеграциями и базами данных (полный доступ)")]
    H711,

    #[serde(rename = "Просматривать базу контрагентов, лиды и воронку продаж")]
    H810,
    
    #[serde(rename = "Управлять сделками, выставлять счета на оплату и согласовывать скидки (полный доступ)")]
    H811,

    #[serde(rename = "Просматривать заявки на закупку и прайс-листы поставщиков")]
    H910,
    #[serde(rename = "Формировать заказы поставщикам, участвовать в тендерах и закупках (полный доступ)")]
    H911,

    #[serde(rename = "Просматривать маркетинговые отчеты и аналитику кампаний")]
    H1010,
    #[serde(rename = "Управлять рекламными бюджетами, подрядчиками и маркетинговыми акциями (полный доступ)")]
    H1011,

    #[serde(rename = "Неизвестное полномиче")]
    Unknown,
}
