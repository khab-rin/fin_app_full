CREATE TABLE IF NOT EXISTS companys (
    ...
    CONSTRAINT is_kpp
        CHECK (kpp ~ '^[0-9]{9}$' OR LENGTH(inn) = 12),
...
CREATE UNIQUE INDEX IF NOT EXISTS inn_kpp_ind ON companys(inn, kpp);
...
BEGIN
    ...
    IF (TG_OP = 'UPDATE') THEN
        IF (NEW.inn <> OLD.inn) or (NEW.comp_type <> OLD.comp_type) THEN
            RAISE EXCEPTION 'Запрещено менять ИНН или Тип организации';
        END IF;
    END IF;


    IF LENGTH(NEW.inn) = 12 THEN
