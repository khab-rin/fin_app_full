...
use shared_lib::sql_models::operation::parser::ParsedBlock;
...
use crate::sql_queries::contracts::get::contracts_by_ids::get_contracts_by_comp_ctrpty_ids;

pub(crate) async fn make_statement_operation_raw(
    state: &ClientState,
...
pub(crate) async fn make_statement_rec_operation_raw(
    state: &ClientState,
    parsed_block: &ParsedBlock
) -> Result<OperationRaw, Status> {
...
pub(crate) async fn make_statement_home_operation_raw(
    state: &ClientState,
    parsed_block: &ParsedBlock
) -> Result<OperationRaw, Status> {
