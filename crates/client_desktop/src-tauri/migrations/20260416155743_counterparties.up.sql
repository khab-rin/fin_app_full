CREATE TABLE IF NOT EXISTS companys (
    ...
    CASE 
        WHEN length(inn) = 12 THEN kpp = '0'
        WHEN length(inn) = 10 THEN length(kpp) = 9
        ELSE 0
    END),
    ...
    CONSTRAINT check_inn_numeric CHECK (inn GLOB '[0-9]*'),
...
CREATE INDEX IF NOT EXISTS inn_kpp_index ON companys(inn, kpp);
